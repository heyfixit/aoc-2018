...
function calcRootValue(node) {
  let sum = 0;
  if(node.meta_count > 0) {
    if(node.child_count === 0) {
      return node.meta_data.reduce((accu, cur) => { return accu + cur; }, 0);
    }

    if(node.child_count > 0) {
      for(let i = 0; i < node.meta_count; i++) {
        let this_child = node.children[node.meta_data[i] - 1];
        if(this_child) {
          sum += calcRootValue(this_child);
        }
      }
    }
  }
...
fs.readFile('input.txt', 'utf8', (err, data) => {
  ...
  console.log(calcRootValue(tree));
});
